	...
	<div class = "well">
		...
		<div class = "panel panel-success">	
			...
			<?php
				...
				$f = $q->fetch_array();
				$q1 = $conn->query("SELECT COUNT(*) as total FROM `complaints` where `status` = 'Pending' && `itr_no` = '$f[itr_no]' && `section` = 'Prenatal'") or die(mysqli_error());
				$f1 = $q1->fetch_array();
				$q2 = $conn->query("SELECT COUNT(*) as total1 FROM `complaints` where `status` = 'Pending' && `itr_no` = '$f[itr_no]' && `section` = 'Maternity'") or die(mysqli_error());
				$f2 = $q2->fetch_array();
			?>
