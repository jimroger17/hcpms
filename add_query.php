<?php
	if(ISSET($_POST['save_d'])){
		...
		echo("<script> location.replace('view_dental_record.php');</script>");
		$conn->close();
	}
	if(ISSET($_POST['save_f'])){
		...
		echo("<script> location.replace('view_fecalysis_record.php');</script>");
		$conn->close();
	}
	...
	if(ISSET($_POST['save_r'])){
		...
		echo("<script> location.replace('view_rehabilitation_record.php');</script>");
		$conn->close();
	}
	if(ISSET($_POST['save_s'])){
		...
		echo("<script> location.replace('view_sputum_record.php');</script>");
		$conn->close();
	}
	if(ISSET($_POST['save_x'])){
		...
		$year = date("Y", strtotime("+8 HOURS"));
		$conn = new mysqli("localhost", 'root', '', 'hcpms') or die(mysqli_error());
		$conn->query("INSERT INTO `radiological` 
    (`case_no`, `referred_by`, `clinical_impression`, `room_bed_no`, `type_of_examination`, `date_taken`, `radiologist`, `itr_no`, `user_id`, `month`, `year`) 
    VALUES 
    ('$case_no', '$referred_by', '$clinical_impression', '$room_bed_no', '$type_of_examination', '$date_taken', '$radiologist', '$itr_no', '$user_id', '$month', '$year')")
or die(mysqli_error());
		...
		echo("<script> location.replace('view_xray_record.php');</script>");
		$conn->close();
	}
	if(ISSET($_POST['save_p'])){
		...
		echo("<script> location.replace('view_prenatal_record.php');</script>");
		$conn->close();
	}
	if(ISSET($_POST['save_patient'])){
		...
		echo("<script> location.replace('view_birth_record.php');</script>");
		$conn->close();
	}
	if(ISSET($_POST['save_birth'])){
		...
		echo("<script> location.replace('view_birthing_record.php');</script>");
		$conn->close();
	}	